...
for TASK in {pcam_10shots,camelyon16_small,panda_small,}; do
    RAY_ADDRESS="https://ray.nebul.prd.kaiko.ai" kray job submit --no-wait --runtime-env=<(echo "$ENV") \
...
for TASK in {bach,crc,mhist,patch_camelyon,camelyon16_small,panda_small,breakhis,bracs,gleason_arvaniti}; do
    ...
    rm -rf /mnt/vast01/shared/experimental/pathology_fm/mikhail/data/eva/EMBEDDINGS/${TASK}/${MODEL_NAME}; \
    MODEL_NAME=$MODEL_NAME \
    IN_FEATURES=$(python get_dim.py $MODEL_NAME 1) \
    OUTPUT_ROOT=/mnt/vast01/shared/experimental/pathology_fm/mikhail/data/eva/RESULTS/${TASK}/${MODEL_NAME} \
    EMBEDDINGS_ROOT=/dev/shm/mikhail/eva/EMBEDDINGS/${TASK}/${MODEL_NAME} \
    DATA_ROOT=/mnt/vast01/shared/experimental/pathology_fm/mikhail/data/eva/${TASK} \
    NORMALIZE_MEAN=[0.5,0.5,0.5] \
    NORMALIZE_STD=[0.5,0.5,0.5] \
